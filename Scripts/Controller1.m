...
xlabel('Time (s)');
ylabel('Output Y(t)');
title('DS1: Closed-Loop Step Response of Controller 1');
...
title('DS2-4: Disturbance Step Input of Controller 1');

% DS 5
sysctlTc = feedback(sysK1, sysG);
...
grid on;
title("DS5: Integrated Tc for step Input of Controller 1");

% Questions 1
numStep = 5;
sysStep = tf(numStep);
syscl1_q1 = series(sysStep, syscl1);
...
xlabel('Time (s)');
ylabel('Output Y(t)');
title('Q1: Closed-Loop Step Response of Controller 1');
...
ylabel('Signal Tc(t)');
grid on;
title("Q2: Signal Tc of Controller 1 for Step Input");
...
grid on;
title("Q3: Disturbance Input of Controller 1");
...
ylabel('Output Y(t) (Degree)');
grid on;
title("Q3: Disturbance Step Input of Controller 1");

% Question 4
sysctlTCTD = feedback(series(-sysG, sysK1), 1, 1);
...
ylabel('Signal Tc(t)');
grid on;
title("Q4: Tc for cosin disturbance input of Controller 1");
...
ylabel('Signal Tc(t)');
grid on;
title("Q4: Disturbance Step Input of Controller 1");
